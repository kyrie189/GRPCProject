...
type TaskServiceClient interface {
	...
	UploadFile(ctx context.Context, opts ...grpc.CallOption) (TaskService_UploadFileClient, error)
}
...
func (c *taskServiceClient) UploadFile(ctx context.Context, opts ...grpc.CallOption) (TaskService_UploadFileClient, error) {
	stream, err := c.cc.NewStream(ctx, &TaskService_ServiceDesc.Streams[0], TaskService_UploadFile_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &taskServiceUploadFileClient{stream}
	return x, nil
}

type TaskService_UploadFileClient interface {
	Send(*FileInfo) error
	CloseAndRecv() (*UploadFileResponse, error)
	grpc.ClientStream
}

type taskServiceUploadFileClient struct {
	grpc.ClientStream
}

func (x *taskServiceUploadFileClient) Send(m *FileInfo) error {
	return x.ClientStream.SendMsg(m)
}

func (x *taskServiceUploadFileClient) CloseAndRecv() (*UploadFileResponse, error) {
...
type TaskServiceServer interface {
	...
	UploadFile(TaskService_UploadFileServer) error
...
func (UnimplementedTaskServiceServer) UploadFile(TaskService_UploadFileServer) error {
...
func _TaskService_UploadFile_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(TaskServiceServer).UploadFile(&taskServiceUploadFileServer{stream})
}

type TaskService_UploadFileServer interface {
	SendAndClose(*UploadFileResponse) error
	Recv() (*FileInfo, error)
	grpc.ServerStream
}

type taskServiceUploadFileServer struct {
	grpc.ServerStream
}

func (x *taskServiceUploadFileServer) SendAndClose(m *UploadFileResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *taskServiceUploadFileServer) Recv() (*FileInfo, error) {
	m := new(FileInfo)
