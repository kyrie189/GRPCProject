...
func (r *Resolver) start() (chan<- struct{}, error) {
	...
	resolver.Register(r) // 为什么这里又注册一次
...
func (r *Resolver) update(events []*clientv3.Event) {
	for _, ev := range events {
		...
		case clientv3.EventTypePut:
			...
			addr := resolver.Address{Addr: info.Addr, Metadata: info.Weight} // 更新地址
...
func (r *Resolver) sync() error {
	...
	r.srvAddrsList = []resolver.Address{}    // 返回resolver的地址

	for _, v := range res.Kvs {  // 所有地址	添加进去
		info, err := ParseValue(v.Value)   // 反序列化
		if err != nil {
			continue
		}
	...
	r.cc.UpdateState(resolver.State{Addresses: r.srvAddrsList})  // 更新地址状态
	return nil
}
