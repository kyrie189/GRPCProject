...
import (
	...
	"github.com/CocaineCong/grpc-todolist/app/lottery/internal/service"
...
func main() {
	...
	for _,v:= range config.Conf.Services["lottery"].Addr{
		...
		userNode := discovery.Server{
			Name: config.Conf.Domain["lottery"].Name,
			Addr: grpcAddress,
		}
		// grpc服务
		...
		defer server.Stop()
		// 绑定service
		pb.RegisterLotteryServiceServer(server, service.GetLotterySrv())
